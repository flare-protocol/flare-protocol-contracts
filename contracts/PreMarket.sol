    ...
    ReentrancyGuardUpgradeable
{
    ...
    function settleFilled(uint256 orderId) public nonReentrant {
        ...
        // transfer order token to contract
        iToken.transferFrom(order.seller, address(this), tokenAmount);
        // transfer token after fee to buyer
        iToken.transfer(order.buyer, tokenAmount - tokenAmountFee);
        ...
        offer.status = STATUS_ORDER_SETTLE_FILLED;
    ...
    function settleCancelled(uint256 orderId) public nonReentrant {
        ...
        offer.status = STATUS_ORDER_SETTLE_CANCELLED;

        emit SettleCancelled(orderId, totalValue, settleFee, msg.sender);
    }

    // Batch actions
    function forceCancelOrders(uint256[] memory offerIds) external {
        for (uint256 i = 0; i < offerIds.length; i++) {
            forceCancelOrder(offerIds[i]);
        }
    }
